...
export default function TopPanel({imageUrl, title, description, about, project, involved}) {
    ...
    return (
        <div className={classnames('text--center col col--4 padding')}>
            {imgUrl && (
                <div>
                <img className={styles.topPanelImage} src={imgUrl} alt={title} />
                </div>
            )}
            <h3>{title}</h3>
            <p>{description}</p>
            <div className={classnames(styles.topPanelCTA)}>
